...
public class Carte {
    ...
    public boolean[] decide(String imput){
        boolean[] rezultat = new boolean[pagini.length];
        int i=0;
        for (Pagina paginaCurenta : pagini){
            rezultat[i] = paginaCurenta.gettext().contains(imput)&&paginaCurenta.isAreImagini()==false;
            if(paginaCurenta.gettext().contains(imput) && paginaCurenta.isAreImagini() == false){
                rezultat[i]=true;
            }else{
                rezultat[i]=false;
            }
