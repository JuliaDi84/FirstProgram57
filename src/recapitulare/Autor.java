...
public class Autor {
    ...
    public Pagina[] findPaginiByAutor() {
        ...
        for (int i = 0; i < carti.length; i++) {
           Pagina[] paginiCarteCurenta = carti[i].getPagini();
            for (int j = 0; j < paginiCarteCurenta.length; j++) {
                Pagina paginaCurenta = paginiCarteCurenta[j];
                rezultat[k] = paginaCurenta;// k stocheaza rezultatul pe rand ( fara a suprascrie rezultatul);
                k++;
            }
        }
        return rezultat;
